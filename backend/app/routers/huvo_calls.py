...
async def huvo_create_lead(req: HuvoLeadRequest, user: dict = Depends(current_user)):
    ...
    async with engine.begin() as conn:
        ...
        lead_id = (await conn.execute(text(
            ...
            " ORDER BY (stage IN ('converted','future_prospect','rejected','rnr')), created_at DESC LIMIT 1"),
            {"p": phone10})).scalar()
        ...
        # `lead_created` is written by the leads_lead_created DB trigger — and only when
        # a lead really was inserted, which this endpoint used to log even when not.
...
async def huvo_bulk_create_leads(req: BulkHuvoLeadRequest, user: dict = Depends(current_user)):
    ...
    async with engine.begin() as conn:
        ...
        linked = (await conn.execute(text("""
            UPDATE huvo_call_updates h
              ...
              FROM (SELECT DISTINCT ON (right(regexp_replace(phone,'[^0-9]','','g'),10))
                     ...
                     ORDER BY p10, (stage IN ('converted','future_prospect','rejected','rnr')), created_at DESC) l
        ...
        # `lead_created` is written per inserted lead by the leads_lead_created DB
        # trigger (actor from source_meta.created_by). This used to log one row per
        # requested number with a NULL entity_id, whether or not a lead was made.
