...
from app.routers.huvo_calls import LINKED_NO, LINKED_YES, calls_filters
...
def test_no_filters_means_no_clause():
    clause, params = calls_filters(None, None, None, None, None)
    assert clause.strip() == ""
    assert params == {}
...
def test_search_is_bound_never_inlined():
    ...
    clause, params = calls_filters("'; DROP TABLE leads--", None, None, None, None)
...
def test_search_covers_number_name_and_summary():
    ...
    clause, _ = calls_filters("whitefield", None, None, None, None)
    src = _sql(clause)
...
def test_linked_splits_calls_that_have_a_lead_from_those_that_dont():
    ...
    yes, _ = calls_filters(None, None, None, LINKED_YES, None)
    no, _ = calls_filters(None, None, None, LINKED_NO, None)
...
def test_an_unknown_linked_value_is_ignored_rather_than_matching_nothing():
    ...
    clause, _ = calls_filters(None, None, None, "banana", None)
...
def test_duration_uses_the_same_buckets_as_the_bonvoice_log():
    ...
    be a trap."""
    clause, params = calls_filters(None, None, None, None, "1-3 mins")
    src = _sql(clause)
...
def test_the_open_ended_bucket_has_no_upper_bound():
    clause, params = calls_filters(None, None, None, None, "5+ mins")
    assert "duration_sec >=" in _sql(clause)
    assert "dur_hi" not in params


def test_filters_combine_with_and():
    clause, params = calls_filters("rohit", "qualified", "yes", LINKED_NO, "5+ mins")
