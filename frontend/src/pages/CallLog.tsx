...
export default function CallLog() {
  ...
  const { values: f, set, clear } = useFilterValues({ conn: "", by: "", dur: "" });
  const { conn, by, dur } = f;
    ...
    <>
      <div className="section-head" style={{ marginBottom: 10 }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          <FilterBar
            fields={[
              { key: "conn", label: "Outcome", options: ANSWERED },
              { key: "by", label: "Placed by", options: byOptions },
              { key: "dur", label: "Duration", options: DURATION_OPTIONS },
            ]}
            values={f}
            // every filter change resets paging — page 3 of the old result set is meaningless
          ...
          {anyFilter && (
            <button className="btn ghost sm"
              onClick={() => { setQ(""); clear(); setPage(0); }}>Clear</button>
          )}
            ...
            {isLoading ? (
              ...
              items.map((c) => (
                <tr key={`${c.call_id}-${c.leg}`}>
                  <td style={{ fontSize: 12, whiteSpace: "nowrap", fontFamily: "var(--font-mono)" }}>{formatDateTime(c.start_at) || "—"}</td>
                  <td style={{ fontSize: 12.5 }}>
                    {c.lead_id
                      ? <>
                          <LeadLink className="lead-link" id={c.lead_id}>{c.lead_name || "View lead"}</LeadLink>
